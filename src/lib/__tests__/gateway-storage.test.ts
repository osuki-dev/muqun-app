...
describe('normalizeGatewaySshTunnel', () => {
  test('a well-formed tunnel spec passes through, trimming the host', () => {
    expect(normalizeGatewaySshTunnel({ hostId: 'host-9', remoteHost: ' 127.0.0.1 ', remotePort: 23847 })).toEqual({
      hostId: 'host-9',
  ...
  test('anything malformed is dropped', () => {
    ...
    expect(normalizeGatewaySshTunnel({ hostId: '', remoteHost: '127.0.0.1', remotePort: 23847 })).toBeUndefined();
    expect(normalizeGatewaySshTunnel({ hostId: 'h', remoteHost: '', remotePort: 23847 })).toBeUndefined();
    expect(normalizeGatewaySshTunnel({ hostId: 'h', remoteHost: '127.0.0.1', remotePort: 0 })).toBeUndefined();
    expect(normalizeGatewaySshTunnel({ hostId: 'h', remoteHost: '127.0.0.1', remotePort: 70000 })).toBeUndefined();
    expect(normalizeGatewaySshTunnel({ hostId: 'h', remoteHost: '127.0.0.1', remotePort: 1.5 })).toBeUndefined();
  });
});
...
describe('back-compat on load', () => {
  ...
  test('a tunnelled record round-trips its tunnel spec', async () => {
    await saveGateway(PAYLOAD, 'Build box', { hostId: 'host-9', remoteHost: '127.0.0.1', remotePort: 23847 });
    const [record] = await loadGateways();
    expect(record.sshTunnel).toEqual({ hostId: 'host-9', remoteHost: '127.0.0.1', remotePort: 23847 });
  ...
  test('a record whose stored sshTunnel is malformed loads as a plain direct gateway', async () => {
    ...
    await saveGateway(
      { ...PAYLOAD },
      'Build box',
      { hostId: 'host-9', remoteHost: '127.0.0.1', remotePort: 999999 } as never
    );
