...
export function emulator(columns: number, rows: number, options: EmulatorOptions = {}): TerminalEmulator {
...
export function lineText(frame: TerminalFrame, row: number): string {
  ...
  if (!line) return '';
  return line.cells.filter((cell) => cell.width > 0).map((cell) => cell.text).join('');
}

export function styleAt(frame: TerminalFrame, row: number, column: number): TerminalStyle | undefined {
